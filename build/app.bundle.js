...
function getNodeindex(elm) {
  return [].concat(_toConsumableArray(elm.parentNode.children)).indexOf(elm);
}
...
function addRow() {
  ...
  newCell.innerHTML = '-';
  for (var i = 1; i < tableRef.rows[1].cells.length - 1; i++) {
...
function addCell() {
  ...
  newCell.classList.add('field');
  for (var i = 2; i < tableRef.rows.length - 1; i++) {
...
function delRow(event) {
  var index = getNodeindex(event.target.parentNode);
  if (actionsLg > 1) {
    actionsLg = actionsLg - 1;
    tableRef.deleteRow(index);
  }
  if (index == 1) {
    var addNewCell = tableRef.rows[1].insertCell(-1);
...
function delCell(event) {
  var index = getNodeindex(event.target);
  if (actionsCellLg > 1) {
    actionsCellLg = actionsCellLg - 1;
    ...
    if (index == 0) {
      ...
      var addNewRow = newRow.insertCell(0);
      ...
      for (var _i = 1; _i < tableRef.rows.length - 1; _i++) {
        tableRef.rows[_i].deleteCell(index + 1);
      }
    }
  }
}
...
tableRef.addEventListener("mouseover", function (event) {
  ...
  if (target.nodeName == 'TD' && target.className == 'field') {
    ...
    for (var _i2 = 0; _i2 < actionsCell.length; _i2++) {
      if (actionsCell[_i2].classList.contains('show')) {
        actionsCell[_i2].classList.remove('show');
      }
    }
    if (actionsLg > 1) {
